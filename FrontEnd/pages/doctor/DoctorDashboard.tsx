...
const DoctorDashboard: React.FC = () => {
  ...
  const doctorAppointments = appointments.filter(
    (apt) => apt.doctorId === user?.id || apt.doctorId === "d1"
  );
  ...
  const totalScheduled = doctorAppointments.filter(
    (apt) => apt.status === "scheduled"
  ).length;
  const totalCompleted = doctorAppointments.filter(
    (apt) => apt.status === "completed"
  ).length;
  const totalCancelled = doctorAppointments.filter(
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          ...
          <Card
            ...
            className="border border-gray-200 mt-6"
          >
