...
const Prescriptions: React.FC = () => {
  ...
  const patientPrescriptions = prescriptions.filter(
    (rx) => rx.patientId === user?.id || rx.patientId === "p1"
  );
  const activePrescriptions = patientPrescriptions.filter(
    (rx) => new Date(rx.endDate) >= new Date()
  );
  const expiredPrescriptions = patientPrescriptions.filter(
    (rx) => new Date(rx.endDate) < new Date()
  );
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          {activePrescriptions.length > 0 ? (
            ...
            <Card className="text-center py-6 border border-gray-200">
